...
public class SignupActivity extends AppCompatActivity {
    ...
    public void Signup(String firstName, String lastName, String address, String phoneNumber, String email, String password) {
        mauth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if (task.isSuccessful()) {
                    // Success: Get user ID from Firebase Authentication
                    String uid = mauth.getCurrentUser().getUid();
                    ...
                    // Store user data under "Users" node using the UID
                    mdatabase.child("Users").child(uid).setValue(pojo).addOnCompleteListener(new OnCompleteListener<Void>() {
                        @Override
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                Toast.makeText(SignupActivity.this, "User Created and Saved: " + uid, Toast.LENGTH_LONG).show();
                                // Navigate to home page
                                NavigateToHomePage();
                            } else {
